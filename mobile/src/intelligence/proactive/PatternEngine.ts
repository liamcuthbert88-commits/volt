import type { AttributeValue, Entity } from "../../world/Entity";
 ...
 * `world/EntityTypes.ts`: the built-in rules below cover the examples this
 * sprint names, and `registerPatternRule` lets a future module add a new
 * kind of regularity to look for without touching this file.
 */
export interface PatternRule {
  readonly id: string;
  readonly kind: PatternKind;
  readonly description: string;
  detect(world: World): readonly Pattern[];
}
...
function lastTouchedAt(world: World, entity: Entity): number {
...
// Integration Sprint 1: the app today only ever produces Trace and
// Conversation entities (see useTraces.ts / ConversationMode.tsx) — none
// of the other built-in rules above have anything to detect against real
// data yet. This one does: it's the existing createStreakRule factory,
// parameterized for the entity type VOLT actually generates, so Proactive
// Intelligence has a real signal to find rather than running against an
// empty World.
registerPatternRule(
