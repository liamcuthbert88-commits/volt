...
import type { Relationship, RelationshipKind } from "./Relationship";
import type { World } from "./World";

/**
 * The read side of the Living World. Every function here is pure — none
 * of them mutate `world`, and none of them exist on the World class
 * itself. Keeping queries separate from World's mutation API means the
 * set of ways to *ask* the World something can grow indefinitely without
 * World.ts ever growing to match — a query is just a function of
 * `(World, ...params) -> typed result`.
 *
...
export function getOutgoingRelationships(world: World, entityId: EntityId): readonly Relationship[] {
  return world.listRelationships().filter((relationship) => relationship.fromId === entityId);
}

export function getIncomingRelationships(world: World, entityId: EntityId): readonly Relationship[] {
...
export function getTouchingRelationships(world: World, entityId: EntityId): readonly Relationship[] {
...
export function getOtherEntity(world: World, entityId: EntityId, relationship: Relationship): Entity | undefined {
  const otherId = relationship.fromId === entityId ? relationship.toId : relationship.fromId;
  return world.getEntity(otherId);
}

export function getNeighborEntities(world: World, entityId: EntityId): readonly Entity[] {
...
export function findEntitiesByType(world: World, type: EntityType): readonly Entity[] {
  return world.listEntities().filter((entity) => entity.type === type);
}

export function findRelationshipsByKind(world: World, kind: RelationshipKind): readonly Relationship[] {
...
export function findConversationsMentioning(world: World, targetId: EntityId): readonly Entity[] {
 ...
 *  of the query. */
export function findRecentEmotionalChanges(world: World, sinceMs: number): readonly Entity[] {
 ...
 *  as incomplete). */
export function findIncompleteMissions(world: World): readonly Entity[] {
...
export function findHabitsAffectingWorkouts(world: World): readonly HabitWorkoutInfluence[] {
